...
def infer_version_from_digital_inputs_file(digital_inputs_file: str | os.PathLike):
    # FIXME DOCME
    with open(digital_inputs_file, "r") as fH:
        line = fH.readline()

    if "Value.Value" in line:
        version = "old"
    else:
        version = "very_old"
    return version

def from_raw_neurophotometrics_file_to_raw_df(path: str | Path, validate=True, version='new') -> pd.DataFrame:
...
def from_raw_neurophotometrics_df_to_ibl_df(raw_df: pd.DataFrame, rois=None, drop_first=True) -> pd.DataFrame:
    ...
    timestamp_name = 'SystemTimestamp' if 'SystemTimestamp' in raw_df.columns else 'Timestamp'
...
def from_raw_neurophotometrics_file_to_ibl_df(path: str | Path, drop_first=True, validate=True, version='new') -> pd.DataFrame:
    ...
    """
    raw_df = from_raw_neurophotometrics_file_to_raw_df(path, validate=validate, version=version)
...
def from_raw_neurophotometrics_file(path: str | Path, drop_first=True, validate=True, version='new') -> dict:
    ...
    Returns:
        dict: _description_
    """
    ibl_df = from_raw_neurophotometrics_file_to_ibl_df(path, drop_first=drop_first, validate=validate, version=version)
    # data_columns = infer_data_columns(ibl_df) if data_columns is None else data_columns
    read_config = dict(
        # data_columns=data_columns,
        time_column='times',
...
def read_digital_inputs_file(path: str | Path, version='new', validate=True) -> pd.DataFrame:
    if type(path) is str:
        path = Path(path)
    if path.suffix == '.csv':
...
def read_digital_inputs_csv(path: str | Path, version='new', validate=True, channel=None) -> pd.DataFrame:
    match version:
        case 'new':
...
def infer_data_columns(df: pd.DataFrame) -> list[str]:
    ...
    data_columns = [col for col in df.columns if col.startswith('Region') or col.startswith('G')]
    return data_columns
